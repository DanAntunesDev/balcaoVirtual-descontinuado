...
import { getMe, updateMe, changeMyPassword, deleteMyAccount } from "@/services/userService";
...
import "@/components/modals/agendamentoModal.css";
...
function resolveBackendUrl(url) {
  ...
  const apiBase = String(import.meta.env.VITE_API_BASE_URL || "http://127.0.0.1:8000/api").replace(/\/+$/, "");
...
export default function MeuPerfil() {
  ...
  // histórico / docs
  const [historico, setHistorico] = useState({ agendamentos: [], atendimentos: [], documentos: [] });
    ...
    (async () => {
      ...
      try {
        ...
        setHistorico({
          ...
          atendimentos: Array.isArray(hist?.atendimentos) ? hist.atendimentos : [],
          documentos: Array.isArray(hist?.documentos) ? hist.documentos : [],
        });

        setCategorias(Array.isArray(cats) ? cats : []);

        const firstAg = Array.isArray(hist?.agendamentos) && hist.agendamentos[0]?.id ? String(hist.agendamentos[0].id) : "";
        setUploadAgendamentoId(firstAg);
      } catch (e) {
  ...
  const ultimosAgendamentos = useMemo(() => {
    return [...agendamentos].sort((a, b) => new Date(b.data_hora) - new Date(a.data_hora)).slice(0, 6);
  }, [agendamentos]);
  ...
  async function enviarDocumento() {
    if (!uploadAgendamentoId) return showToast("error", "Selecione um agendamento.");
    if (!uploadCategoriaId) return showToast("error", "Selecione uma categoria.");
    if (!uploadArquivo) return showToast("error", "Selecione um arquivo.");
  ...
  async function confirmarExcluirConta() {
    setDeleting(true);
    try {
      await deleteMyAccount({ confirmText: deleteText });
    ...
    <div className="mp-wrap">
      ...
      <div className="mp-controls">
        <div className="mp-dropdown" ref={menuRef}>
          <button className="mp-dd-btn" type="button" onClick={() => setMenuOpen((v) => !v)}>
          ...
          {menuOpen && (
            <div className="mp-dd-menu">
              {SECTIONS.map((s) => (
                ...
                >
                  {s.label}
                  {section === s.key && <span className="material-symbols-outlined">check</span>}
                </button>
              ))}
            </div>
          )}
          ...
          {section === "seguranca" && (
            <>
              ...
              <div className="mp-actions mp-actions--between">
                <button type="button" className="mp-btn" disabled title="Requer estratégia de revogação de tokens">
                  Encerrar Sessão em outros dispositivos
                </button>

                <button type="button" className="mp-btn mp-btn--danger" onClick={() => setDeleteOpen((v) => !v)}>
                  Excluir conta
                </button>
              </div>
              ...
              {deleteOpen && (
                <div className="mp-danger-box">
                  <p className="mp-danger-title">Confirmação obrigatória</p>
                  <p className="mp-danger-sub">
                    Digite <strong>EXCLUIR</strong> para desativar sua conta.
                  </p>
            ...
            <>
              ...
              <div className="mp-split">
                <div className="mp-block">
                  <h3>Ultimos Agendamentos Realizados</h3>
                ...
                <div className="mp-upload">
                  <div className="bv-doc-item">
                    ...
                    <div className="bv-field bv-select-wrap">
                      ...
                      <select className="bv-tag" value={uploadCategoriaId} onChange={(e) => setUploadCategoriaId(e.target.value)}>
                        <option value="">Selecione...</option>
                        {categorias.map((c) => (
                    ...
                    <div className="bv-field bv-file-wrap">
                      ...
                      <label className="bv-file-btn" htmlFor={fileInputId}>
                        ...
                        <span className="bv-file-action">{uploadArquivo ? "Trocar" : "Selecionar"}</span>
                      </label>
                    </div>

                    <button type="button" className="bv-trash" onClick={clearUpload} disabled={uploading} aria-label="Limpar" title="Limpar">
                      <span className="material-symbols-outlined">delete</span>
                    </button>
                  </div>

                  <div className="mp-actions">
                    <button type="button" className="mp-btn" onClick={() => { setUploadOpen(false); clearUpload(); }} disabled={uploading}>
                      Cancelar
                    </button>

                    <button type="button" className="mp-btn mp-btn--primary" onClick={enviarDocumento} disabled={uploading}>
                    ...
                    return (
                      <div className="mp-doc" key={d.id}>
                        <div className="mp-doc-left">
                          ...
                          {normalize(d.status).includes("reprov") && d.observacao_validacao ? (
                            <div className="mp-doc-note">{d.observacao_validacao}</div>
                          ) : null}
                        </div>
